...
```
    ...
    - Logaritmos: `log()`, `log2()`, `log10()`, para trabalhar com dados cujo
    valor varia muito.
    ...
    quando uma variável muda de valor `x != lag(x)`.
...
```
    ...
    - Funções de agregação e valores acumulados: `cumsum()`, `cumprod()`, 
...
```
    ...
    - Se ela não servir, tente `row_number()`, `dense_rank()`, 
    `percent_rank()`, `cume_dist()` ou `ntile()`.
...
Quando você plota as duas variáveis, acontece algo similar:
    ...
    necessariamente os melhores jogadores, são apenas "sortudos".
...
Já falamos de contagens (`n()`) e médias (`mean()`), que são extremamente úteis, mas há outras funções para medidas resumo que são interessantes.
    ...
    uma agregação e um filtro ao mesmo tempo.
...
```
    ...
    - Medidas de dispersão: `sd()`, `IQR()`, `mad()`. A primeira é o desvio
    ...
    úteis quando você tem dados com valores muito discrepantes (outliers).
...
```
    ...
    - Medidas de ranking: `min()`, `quantile()`, `max()`. Quantile é uma
    generalização da mediana. `quantile(x, 0.25)` encontra x que são maiores
    que 25% e menores que 75% dos valores.
...
```
    ...
    - Medidas de posição: `first()`, `nth()`, `last()`. "Primeiro", "enésimo"
    ...
    a última decolagem em cada dia.
...
```
    ...
    Isso é corresponde a você fazer um filtro a partir de uma variável de
    ...
    só com as variáveis que você criou.
...
```
    ...
    - Contagens: `n()` conta o número de observaçõs no grupo atual, não recebe
    nenhum argumento, se você quiser contar só os missings, use `sum(is.na(x))`,
    para contar o número de valores únicos/distintos, use `n_distinct(x)`.
...
```
    ...
    `count()` aceita um argumento opcional peso (`wt`), 
    para ponderar as contagens.
...
# Exercícios
    ...
    2. Há uma função auxiliar de `filter()` chamada `between()`. 
    O que ela faz? Como você poderia utilizá-la para simplificar
    alguns dos testes feitos anteriormente?
    3. Quantos voos tem valor missing na variável `dep_time`? 
    Que outras variáveis tem missings? O que esses missings significam?
    4. Porque `NA ^ 0` não é `NA`? Porque `NA | TRUE` não é `NA`? 
    Porque `FALSE & NA` não é `NA`? Qual é a "regra geral" para esses casos? 
    (Ignore o caso NA * 0, que é um pouco mais complicado)
    5. Como você poderia reordenar (`arrange()`) o banco para que
    os valores missing viessem primeiro? Dica: `is.na()`.
    6. Reordene `flights` para encontrar os voos mais atrasados. 
    Encontre os voos que saíram mais cedo.
    7. Reordene `flights` para encontrar os voos mais rapidos 
    (velocidade em voo).
    8. Quais voos viajaram para mais longe e para mais perto?
    9. Imagine todas as possibilidades que você pode utilizar com `select()`
    para escolher as variáveis `dep_time`, `dep_delay`, `arr_time` 
    e `arr_delay`.
    10. O que acontece se você colocar o nome de uma mesma variável 
    várias vezes dentro de um chamado da função `select()`.
    11. O que faz o helper `one_of()`? Porque ela poderia ser útil 
    se utilizada com o seguinte vetor?
    
```{r}
vars <- c("year", "month", "day", "dep_delay", "arr_delay")
```
    
    12. Execute o código abaixo. O resultado te surpreendeu? Como os helpers
    de `select()` entendem o uso da maiúsculas por padrão? Como você poderia 
    alterar esse comportamento?
    13. Atualmente, `dep_time` e `sched_dep_time` são convenientes para 
    apresentação, mas são ruins de usar para cálculos, pois não são números
    na forma de variável contínua. Converta-os para uma representação do 
    número de minutos passados desde a meia-noite.
    14. Compare `air_time` com `arr_time - dep_time`. O que você esperaria ver?
    O que você vê? O que você precisa fazer para consertar este problema?
    15. Compare `dep_time`, `sched_dep_time` e `dep_delay`. Como você 
    esperaria que estes três números se relacionassem?
    16. Encontre os 10 voos mais atrasados usando uma função da família 
    `_rank()`. Como você lida com os "empates". Leia a documentação de
    `min_rank()`.
    17. O que 1:3 + 1:10 retorna? Porque?
    18. Que funções trigonométricas estão disponíveis em R?
    19. Imagine pelo menos 5 formas diferentes de medir as características
    dos atrasos em grupos de voos. Considere os seguintes cenários:
      * Um voo atrasa 50% das vezes e 50% das vezes não atrasa
      * Um voo sempre atrasa 10 minutos
      * Um voo atrasa 30 minutos 50% das vezes e chega 30 minutos mais cedo
      nas outras 50%
      * 99% o voo chega no horario e 1% das vezes atrasa duas horas
    Qual variável é mais relevante, atraso na chegada (`arr_delay`) ou 
    atraso na decolagem (`dep_delay`)?
    20. Desenvolva um código que produza o mesmo resultado de 
    `not_cancelled %>% count(dest)` e 
    `not_cancelled %>% count(dest, wt = distance)`, mas sem usar `count()`.
    21. Nossa definição de voos cancelados 
    ( is.na(dep_delay) | is.na(arr_delay) ) tem um pequeno problema. 
    Porque? Qual é a coluna mais importante?
    22. Olhe o número de voos cancelados por dia. Há algum padrão? 
    A proporção de voos cancelados tem alguma relação com
    a média do tempo de atraso?
    23. Qual a companhia área que tem os piores atrasos? 
    Desafio: você consegue descobrir se os atrasos são culpa da Cia. Aérea ou
    do Aeroporto? Porque (não)? 
    Dica: reflita sobre `flights %>% group_by(carrier, dest) %>% summarise(n())`
    24. O que argumento `sort` faz em `count()`? Quando você o utilizaria?
    25. Volte para as listas de funções que podem ser utilizadas com `mutate()`
    e `filter()`. Descreve como cada operação muda a partir do momento em que
    você usa um agrupamento (`group_by()`).
    26. Qual avião (`tailnum`) tem a pior pontualidade?
    27. Qual o horário do dia no qual você deveria viajar para minimizar a
    possibilidade de atrasos?
    28. Para cada destino, compute o número total de minutos de atraso. 
    Para cada voo, compute a proporção de atraso total até o seu destino.
    29. Atrasos são geralmente correlacionados temporalmente: mesmo após o
    problema ser resolvido, voos posteriores atrasam para permitir que os voos
    represados decolem. Usando `lag()`, explore como o atraso de um voo 
    se relaciona com o atraso de um voo imediatamente anterior.
    30. Analise os destinos. Você pode encontrar voos que são
    rápidos-demais-pra-ser-verdade (ou seja, podem ter sido registrados
    incorretamente no banco)? Compute o tempo de voo relativo ao tempo de voo
    mais curto registrado para cada destino. Que aviões atrasaram mais durante
    o trajeto?
    31. Encontre todos os destinos operadores por pelo menos duas cias. aéreas.
    Use está informação para ordená-las.
    32. Para cada avião, conte o número de voos antes do primeiro atraso maior
    que 1h.
